...
from evaluation.visualization import create_hpo_plots, create_loss_plot, create_prediction_visualization
from models.linda_pinn import LINDAPINNModel, device
from training.hyperparam_search import run_hpo
...
def run_hpo_gradio(use_synthetic_data, n_trials, epochs_per_trial):
    """Gradio wrapper for HPO — returns (results_text, hpo_fig)."""
    try:
        best_params, study, results_text = run_hpo(
            use_synthetic_data=use_synthetic_data,
            n_trials=int(n_trials),
            epochs_per_trial=int(epochs_per_trial),
        )
        hpo_fig = create_hpo_plots(study)
        return results_text, hpo_fig
...
def create_gradio_app():
    with gr.Blocks(title="LINDA vs LINDA-PINN Comparison") as app:
        ...
        with gr.Tabs():
            # ------------------------------------------------------------------
            # Tab 1: Model Comparison (existing)
            # ------------------------------------------------------------------
            ...
            # ------------------------------------------------------------------
            # Tab 2: Hyperparameter Optimization (new — task 4.1)
            # ------------------------------------------------------------------
            with gr.Tab("Hyperparameter Optimization"):
                gr.Markdown("""
                ## PINN Hyperparameter Search (Task 4.1)

                Uses **Bayesian optimization** (Optuna TPE sampler) to find the best combination of:
                - Learning rate
                - Hidden layer size
                - Number of layers
                - Physics loss weight
                ...
                Each trial trains the PINN for `Epochs per Trial` epochs and returns the average
                loss over the last quarter of training as the objective.
                """)

                with gr.Row():
                    with gr.Column():
                        hpo_use_synthetic = gr.Checkbox(value=True, label="Use Synthetic Data (faster)")
                        hpo_n_trials = gr.Slider(5, 100, value=20, step=5, label="Number of Trials")
                        hpo_epochs = gr.Slider(5, 50, value=10, step=5, label="Epochs per Trial")
                        hpo_run_btn = gr.Button("Run Hyperparameter Search", variant="primary")

                with gr.Row():
                    hpo_results = gr.Markdown(value="*Results will appear here after the search completes.*")

                with gr.Row():
                    hpo_plot = gr.Plot(label="Optimization History & Parameter Importance")

                hpo_run_btn.click(
                    fn=run_hpo_gradio,
                    inputs=[hpo_use_synthetic, hpo_n_trials, hpo_epochs],
                    outputs=[hpo_results, hpo_plot],
                )
